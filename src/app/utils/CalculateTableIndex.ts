        ...
        let number =
            pagingArgs.sortBy == "id"
                ? pagingArgs.sortDirection == "desc"
                    ? (pagingArgs.pageNumber - 1) * pagingArgs.pageSize +
                      index +  1
                    : pagingArgs.pageNumber * pagingArgs.pageSize <
                          totalElements && pagingArgs.pageSize != -1
                    ? (totalPages - pagingArgs.pageNumber) *
                          pagingArgs.pageSize -
                      index + 2
                    : (totalPages - pagingArgs.pageNumber) *
                          pagingArgs.pageSize -
                ...
                : pagingArgs.pageNumber * pagingArgs.pageSize < totalElements &&
                  pagingArgs.pageSize != -1
                ? (totalPages - pagingArgs.pageNumber) * pagingArgs.pageSize -
                  index +  2
                : (totalPages - pagingArgs.pageNumber) * pagingArgs.pageSize -
                  index +  totalElements;
        return number;
